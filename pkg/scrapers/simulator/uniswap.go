...
type UniSimulator struct {
	Eth *ethclient.Client
	log *logrus.Logger
}

func NewUniSimulator(client *ethclient.Client, log *logrus.Logger) *UniSimulator {
	c := UniSimulator{Eth: client, log: log}
	return &c

}

func (c *UniSimulator) Execute(t1 models.Asset, t2 models.Asset) (string, error) {
...
func (c *UniSimulator) quoteTokens(input string, token0 *coreEntities.Token, token1 *coreEntities.Token) (string, error) {
